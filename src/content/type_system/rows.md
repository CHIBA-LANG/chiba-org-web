...
level-1 的 field access、method receiver shape、structural generic 参数，以及 `dyn {r | ...}` 的静态 shape contract，通常都依赖 open row。
...
Field access, method receiver shapes, structural generic parameters, and the static contract carried by `dyn {r | ...}` in level-1 usually depend on open rows.
...
Rows do not choose methods or resolve operator overloads on their own, and they do not prove behavior sources. They provide the normalized receiver shape, field information, and part of the cache key used by shaped dispatch and dynamic-row checking.
