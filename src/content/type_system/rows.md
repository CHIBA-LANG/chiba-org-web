...
1. stored field：record field 或 nominal row field 直接提供成员。
2. adapter member：concrete nominal receiver method 在实例化 / dyn package injection 时被绑定为 adapter。
...
1. a stored field, either from a record field or a nominal row field;
2. an adapter member, where a concrete nominal receiver method is bound at instantiation or dynamic-package injection time.
